...
for(j=0;j<5;j++){
    if(j==0){ 
        ...
        x2old = x_iteracion2;
        x_iteracion2 = ((-A[1][0]*x1old)-(A[1][2]*x_iteracion3)+b[1])/A[1][1];
        Ea2=fabs((x_iteracion2-x2old)/(x_iteracion2))*100;
        x3old = x_iteracion3;
        x_iteracion3 = ((-A[2][0]*x1old)-(A[2][1]*x2old)+b[2])/A[2][2];
        Ea3=fabs((x_iteracion3-x3old)/(x_iteracion3))*100;
        printf("x_1 La iteracion %d es igual a %.4f, Con un error del %.4f %%\n",j+1,x_iteracion1,Ea1);
        printf("x_2 La teracion %d es igual a %.4f, Con un error del %.4f %%\n",j+1,x_iteracion2,Ea2);
        printf("x_3 La iteracion %d es igual a %.4f, Con un error del %.4f %%\n",j+1,x_iteracion3,Ea3);
        }
    }
}